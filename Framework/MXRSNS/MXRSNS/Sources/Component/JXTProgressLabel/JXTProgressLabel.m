...
#pragma mark - 进度控制视图高度
- (void)setClipHeight:(CGFloat)clipHeight
{
    CGRect rect = self.clipView.frame;
    rect.size.height = self.bounds.size.height - clipHeight;//从底部开始
    self.clipView.frame = rect;
}

- (CGFloat)clipHeight {
    return self.clipView.frame.size.height;
}
