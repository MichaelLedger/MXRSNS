...
#pragma mark - 生命周期相关
- (instancetype)initWithFrame:(CGRect)frame imageInfo:(MXRBookSNSUploadImageInfo *)imageInfo imageFrame:(CGRect)imageFrame {
...
- (void)reloadData {
    ...
    if (self.imageInfo.image) {
        self.mainImageView.image = self.imageInfo.image;
        [self adjustUI:NO];
    } else {
        NSString *imageUrl = [self.imageInfo.imageUrl stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
        ...
        NSInteger width = [[NSString stringWithFormat:@"%.0f", self.imageFrame.size.width] integerValue];
        NSInteger height = [[NSString stringWithFormat:@"%.0f", self.imageFrame.size.height] integerValue];
        NSString *compressedImageUrl = [imageUrl stringByAppendingString:[NSString stringWithFormat:@"?imageView2/1/w/%ld/h/%ld/interlace/1", (long)width, (long)height]];
        /*
         https://img.mxrcorp.cn/dynamic/6C64E56D0B474896AF2D8EE3DDD6270E.jpg?imageView2/1/w/108/h/108/interlace/1
...
#pragma mark - UI调整
- (void)adjustUI:(BOOL)animate {
    if (!self.mainImageView.image) return;
    ...
    mxr_dispatch_main_async_safe(^{
        if (animate && _mainImageView.yy_width > 0) {
            [UIView animateWithDuration:0.8f animations:^{
                _mainImageView.yy_width = imageViewWidth;
                _mainImageView.yy_height = imageViewHeight;
                
                _mainScrollView.contentSize = _mainImageView.frame.size;
                _mainImageView.center = [self centerOfScrollViewContent:_mainScrollView];
            }];
        } else {
            _mainImageView.yy_width = imageViewWidth;
            _mainImageView.yy_height = imageViewHeight;
...
//        [self noticeDelegateBeginOrEndDrag:NO];
    ...
    if (self.doingPan && pan.numberOfTouches != 1) {
        return;//拖拽中，用户多指进行骚操作
    }
    ...
    if (self.moveImage == nil)
    {
        ...
        [self bringSubviewToFront:self.moveImage];
        self.moveImage.contentMode = UIViewContentModeScaleAspectFill;
        self.moveImage.backgroundColor = [UIColor blackColor];
        self.moveImage.layer.masksToBounds = YES;
        if (self.imageInfo.image) {
            self.moveImage.image = self.imageInfo.image;
        } else {
            NSInteger width = [[NSString stringWithFormat:@"%.0f", self.imageFrame.size.width] integerValue];
            NSInteger height = [[NSString stringWithFormat:@"%.0f", self.imageFrame.size.height] integerValue];
...
- (void)saveFrameBeginPan
{
    ...
    if (imageWidthBeforeDrag == 0) {
        imageWidthBeforeDrag = SCREEN_WIDTH_DEVICE;
    }
    if (imageHeightBeforeDrag == 0) {
        imageHeightBeforeDrag = SCREEN_HEIGHT_DEVICE;
    }
...
//        CGFloat imageX = -self.mainScrollView.contentOffset.x;
        ...
        CGRect currentRect = [window convertRect:self.mainImageView.frame fromView:self.mainImageView.superview];
        
        [self.delegate YYPhotoBrowserSubScrollViewDoSingleTapWithImageFrame:currentRect];
    }
}
