...
- (CAShapeLayer *)circleLineShapeLayer:(NSInteger)index{
    ...
    shapeLayer.fillColor = _radarCircleBgColor ? [self getNewColorWith:_radarCircleBgColor index:index].CGColor: ZFClear.CGColor;
    shapeLayer.path = [self drawCircle:index].CGPath;
    return shapeLayer;
}
...
/**
 ...
 @return 不同透明度的颜色
 */
- (UIColor *)getNewColorWith:(UIColor *)color index:(NSInteger)index {
    CGFloat red = 0.0;
    CGFloat green = 0.0;
    CGFloat blue = 0.0;
    CGFloat alpha = 0.0;
    [color getRed:&red green:&green blue:&blue alpha:&alpha];
    UIColor *newColor = [UIColor colorWithRed:red green:green blue:blue alpha:(index+1)/(_sectionCount+1.0)/3.0*2];
    return newColor;
}
