...
//    self.radar = [[ZFRadar alloc] init];
    [self addSubview:self.radar];
    
//    ZFRadar *radar = self.radar;
//    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-margin-[radar]-margin-|" options:kNilOptions metrics:@{@"margin":@0} views:NSDictionaryOfVariableBindings(radar)]];
//    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-margin-[radar]-margin-|" options:kNilOptions metrics:@{@"margin":@0} views:NSDictionaryOfVariableBindings(radar)]];
    
//    [self.radar mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)strokePath{
    ...
    self.radar.radarCircleBgColor = _radarCircleBgColor;
...
- (void)setOriginRotationAngle:(CGFloat)originRotationAngle {
    _originRotationAngle = originRotationAngle;
    self.radar.transform = CGAffineTransformMakeRotation(_originRotationAngle);
    for (ZFLabel * itemLabel in self.itemLabelArray) {
        itemLabel.transform = CGAffineTransformMakeRotation(-(_originRotationAngle));
    }
    _finalRotationAngle = _originRotationAngle;
}
